...
public class BaseTaskPool {
	...
	// http post task with params
	public void addTask (int taskId, String taskUrl, String filePath, BaseTask baseTask, int delayTime) {
		baseTask.setId(taskId);
		try {
			TaskThread thread = new TaskThread(context, taskUrl, null, baseTask, delayTime);
			//thread.setFileName(fileName);
			thread.setFilePath(filePath);
			thread.setPostFile(true);
	...
	private class TaskThread implements Runnable {
		...
		public void setPostFile(Boolean file)
		{
			postFile = file;
		}
			...
			try {
				...
				try {
					...
					if (this.taskUrl != null) {
						...
						if(postFile)
						{
							httpResult = client.postFile(filePath);
						}
						else
						{
							// http get
